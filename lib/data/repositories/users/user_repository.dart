...
class UserRepository {
  ...
  Future<void> setUsername(String id, String userName) async {
    ...
    await ref.update({
      UserDocumentPaths.userName: userName.trim(),
  ...
  Future<void> markVerified(String id, bool isVerified) async {
    await _users.doc(id).update({
      UserDocumentPaths.isVerified: isVerified,
      UserDocumentPaths.updatedAt: FieldValue.serverTimestamp(),
    });
  }
  ...
  Future<void> create(model.User u) async {
    ...
    batch.update(ref, {
      ...
      UserDocumentPaths.createdAt: FieldValue.serverTimestamp(),
      UserDocumentPaths.updatedAt: FieldValue.serverTimestamp(),
    });
    await batch.commit();
  }
  ...
  Future<void> renameUsername({
    ...
    final rawUserRef = db.doc(UserDocumentPaths.doc(uid));
    final nameRef = db.doc(UsernameDocumentPaths.doc(desiredLc));
    ...
    await db.runTransaction((tx) async {
      ...
      if (currentLc == desiredLc && current != desired) {
        ...
        if (curMapSnap.exists &&
            (curMapSnap.get(UsernameDocumentPaths.userId) as String?) != uid) {
          throw StateError('Username taken');
        }
        tx.set(nameRef, {
          UsernameDocumentPaths.userId: uid,
          FirestoreFields.createdAt: FieldValue.serverTimestamp(),
        });
        tx.update(rawUserRef, {
          UserDocumentPaths.userName: desired,
          UserDocumentPaths.userNameLower: desiredLc,
          UserDocumentPaths.updatedAt: FieldValue.serverTimestamp(),
        });
        return;
      }
      ...
      if (nameSnap.exists &&
          (nameSnap.get(UsernameDocumentPaths.userId) as String?) != uid) {
        throw StateError('Username taken');
      }
      ...
      if (currentLc.isNotEmpty && currentLc != desiredLc) {
        tx.delete(db.doc(UsernameDocumentPaths.doc(currentLc)));
      }
  ...
  Future<void> _setUsernameUnsafe(String id, String userName) async {
    await _users.doc(id).update({
      UserDocumentPaths.userName: userName.trim(),
      UserDocumentPaths.userNameLower: userName.trim().toLowerCase(),
      UserDocumentPaths.updatedAt: FieldValue.serverTimestamp(),
    });
  }
  ...
  Future<void> setEmail({required String uid, required String email}) async {
    await _users.doc(uid).update({
      UserDocumentPaths.email: email.trim().toLowerCase(),
      UserDocumentPaths.updatedAt: FieldValue.serverTimestamp(),
    });
  }
  ...
  Future<void> updateNamesAndPhone({
    ...
    if (firstName != null) {
      data[UserDocumentPaths.firstName] = firstName.trim();
    }
    if (middleName != null) {
      data[UserDocumentPaths.middleName] = middleName.trim();
    }
    if (lastName != null) {
      data[UserDocumentPaths.lastName] = lastName.trim();
    }
    if (phone != null) {
      data[UserDocumentPaths.phoneNumber] = phone.toJson();
    }
  ...
  Future<void> createUserWithUsername(model.User u) async {
    ...
    await db.runTransaction((tx) async {
      ...
      tx.set(userRef, u);
      tx.update(userRef, {
        UserDocumentPaths.email: u.email.trim().toLowerCase(),
        UserDocumentPaths.userName: desired, // as typed
        UserDocumentPaths.userNameLower: desiredLc, // for lookups
        UserDocumentPaths.createdAt: FieldValue.serverTimestamp(),
        UserDocumentPaths.updatedAt: FieldValue.serverTimestamp(),
      });
...
Map<String, Object?> _userToFirestore(model.User u) {
  ...
  // Normalize
  json[UserDocumentPaths.email] = u.email.trim().toLowerCase();
  json[UserDocumentPaths.userName] = u.userName.trim();

  // Convert date-ish fields to Timestamps for consistency
  json[UserDocumentPaths.birthDate] = Timestamp.fromDate(u.birthDate);
  json[UserDocumentPaths.createdAt] = Timestamp.fromDate(u.createdAt);
  json[UserDocumentPaths.updatedAt] = Timestamp.fromDate(u.updatedAt);

  // Default the flag on write if someone forgot to set it
  json[UserDocumentPaths.isVerified] =
      (json[UserDocumentPaths.isVerified] as bool?) ?? false;
