...
class MapStyle {
  ...
  // NEW: small purple circle backgrounds
  ...
  Future<void> ensure(MapboxMap map) async {
    ...
    // === SMALL OWL-PURPLE CIRCLE BACKGROUNDS ===============================

    // Unclustered BG (small circle)
    if (!await style.styleLayerExists(lyrUnclusteredBg)) {
      ...
      await style.setStyleLayerProperty(
        lyrUnclusteredBg,
        'filter',
        jsonEncode(['all', ['!', ['has', 'point_count']]]),
      );
      // background owl purple (adjust if your constant name differs)
      await style.setStyleLayerProperty(lyrUnclusteredBg, 'circle-color', owlPurple.toHex());
      await style.setStyleLayerProperty(lyrUnclusteredBg, 'circle-radius', 12.0);
      await style.setStyleLayerProperty(lyrUnclusteredBg, 'circle-opacity', 1.0);
      await style.setStyleLayerProperty(lyrUnclusteredBg, 'circle-stroke-width', 0.0);
    }

    // VIP BG (slightly larger)
    if (!await style.styleLayerExists(lyrVipBg)) {
      await style.addLayer(CircleLayer(id: lyrVipBg, sourceId: srcVenuesVip));
      await style.setStyleLayerProperty(lyrVipBg, 'circle-color', owlPurple.toHex());
      await style.setStyleLayerProperty(lyrVipBg, 'circle-radius', 14.0);
      await style.setStyleLayerProperty(lyrVipBg, 'circle-opacity', 1.0);
      await style.setStyleLayerProperty(lyrVipBg, 'circle-stroke-width', 0.0);
    }

    // === FOREGROUND ICONS (INSIDE THE CIRCLE) ==============================
    ...
    if (!await style.styleLayerExists(lyrUnclustered)) {
      ...
      // Verified -> logo inside circle; else -> type icon inside circle
      await style.setStyleLayerProperty(
        lyrUnclustered,
        'icon-image',
        jsonEncode([
          'case',
          ['==', ['get', 'isVerified'], true],
          // NOTE: the value must be an ID of a previously-registered style image.
          // You can set it equal to logo_url and register an image with that same ID.
          ['get', 'logo_url'],
          // Otherwise: your existing per-type icon names (rendered centered inside the purple circle)
          ['case',
      ...
      // Make the inner icon smaller so it sits inside the circle
      await style.setStyleLayerProperty(lyrUnclustered, 'icon-size', 0.75);
      await style.setStyleLayerProperty(lyrUnclustered, 'icon-allow-overlap', true);
      // No halo; circle layer is the background
