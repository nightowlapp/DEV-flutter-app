...
final _fallback = CameraOptions( //TODO make users location.
...
final initialCameraProvider = FutureProvider<CameraOptions>((ref) async {
  final loc = await ref.read(locationControllerProvider.notifier).currentLatLngOrNull();
  if (loc == null) return _fallback;
  return CameraOptions(center: Point(coordinates: Position(loc.lng, loc.lat)), zoom: mapZoomDefault);
});
