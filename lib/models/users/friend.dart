...
class FriendProfile {
  ...
  factory FriendProfile.fromUserDoc(String uid, Map<String, dynamic> data) {
    ...
    final statusRaw =
        data[UserDocumentPaths.currentPartyStatus] as String? ??
...
class FriendEdge {
  ...
  static FriendEdge from(String uid, Map<String, dynamic> m) => FriendEdge(
    ...
    isCloseFriend:
    (m[FriendEdgeFields.isCloseFriend] as bool?) ?? false,
    createdAt: (m[FirestoreFields.createdAt] as Timestamp?)
        ?.toDate() ??
        DateTime.fromMillisecondsSinceEpoch(0),
    updatedAt:
    (m[FirestoreFields.updatedAt] as Timestamp?)?.toDate(),
  );
}
